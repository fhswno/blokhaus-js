...
Scribex provides two serialization formats: **JSON** for persistence and **Markdown** for AI context and interoperability. JSON preserves the full Lexical AST including all formatting, node types, and metadata. Markdown provides a human-readable representation used primarily for sending editor content to AI models.
...
```tsx
import { serializeNodesToMarkdown } from "@scribex/core";
...
```tsx
import { $parseMarkdownToLexicalNodes } from "@scribex/core";
import { $getRoot } from "lexical";
