...
description: Build your own Lexical plugins that integrate with Scribex.
---

Scribex follows a composable plugin architecture. Every feature is an independent React component passed as a child to `EditorRoot`. Plugins render `null` and register Lexical listeners to extend editor behavior. This page explains how to build your own.

## Plugin architecture

A Scribex plugin is a React component that:
...
export function WordCountPlugin({
  ...
  return (
    <div className="text-xs text-scribex-muted-foreground p-2">
      {wordCount} words
    </div>
  );
}
...
All plugins that use hooks or browser APIs must include the `'use client'` directive at the top of the file. Scribex is built for Next.js App Router, where components are server components by default.
