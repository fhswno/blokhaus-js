...
Scribex provides a provider-agnostic AI integration system. You supply an `AIProvider` that returns a `ReadableStream` of text, and Scribex handles the streaming UI, accept/discard flow, and history management.
...
import { EditorRoot, AIPlugin } from '@scribex/core';
...
```tsx
import { OPEN_AI_PROMPT_COMMAND, INSERT_AI_PREVIEW_COMMAND } from '@scribex/core';
