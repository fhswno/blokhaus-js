...
Scribex includes a full-featured table system built on Lexical's `@lexical/table` module. It provides cell merging, column resize, contextual right-click menus, and hover actions for adding rows and columns.
...
Use the `INSERT_TABLE_COMMAND_SCRIBEX` command to insert a table programmatically:
...
|---|---|
| Default (clear) | -- |
| Gray | `--scribex-cell-bg-gray` |
| Brown | `--scribex-cell-bg-brown` |
| Orange | `--scribex-cell-bg-orange` |
| Yellow | `--scribex-cell-bg-yellow` |
| Green | `--scribex-cell-bg-green` |
| Blue | `--scribex-cell-bg-blue` |
| Purple | `--scribex-cell-bg-purple` |
| Pink | `--scribex-cell-bg-pink` |
| Red | `--scribex-cell-bg-red` |
...
Lexical's table module provides a built-in cell selection system. Click and drag across cells to select a range. The selected cells are highlighted with the `--scribex-table-selection` overlay color.
...
```css
:root {
  --scribex-table-border: hsl(214.3 31.8% 91.4%);
  --scribex-table-header-bg: hsl(210 40% 96.1%);
  --scribex-table-selection: hsla(221.2 83.2% 53.3% / 0.1);
  --scribex-table-resize-handle: hsl(221.2 83.2% 53.3%);
}
```
