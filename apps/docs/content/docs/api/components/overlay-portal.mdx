...
```tsx
import { OverlayPortal } from '@scribex/core';
```
...
import { EditorRoot, OverlayPortal, InputRulePlugin } from '@scribex/core';
...
A 2px blue line (`--scribex-accent` color) appears during drag to show the drop position:
...
- Only tracks mouse events relative to its own editor's root element (found via `[data-scribex-root]`)
...
- Native text selection across block boundaries is unaffected by the overlay. Scribex does not wrap blocks in custom container elements -- it uses the "Block Illusion" approach described in the architecture documentation.
