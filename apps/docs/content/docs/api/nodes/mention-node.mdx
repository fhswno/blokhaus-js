...
```tsx
import { MentionNode, $createMentionNode, $isMentionNode } from '@scribex/core';
import type { MentionPayload } from '@scribex/core';
```
...
editor.read(() => {
  ...
  console.log('Found mentions:', mentions.map(m => m.getTextContent()));
});
```
...
- `#` triggers use the `--scribex-accent` token (blue by default).
- All other triggers (including `@`) use the `--scribex-ai-stream` token.
...
import type { MentionProvider, MentionItem } from '@scribex/core';
