...
Blokhaus provides two nodes for video handling: `VideoNode` for permanent video embeds and uploaded files, and `LoadingVideoNode` for optimistic previews during file upload. Together they implement the video upload and embed flow described in the [Video Embeds guide](/docs/guides/video-embeds).
