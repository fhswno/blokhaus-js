...
Blokhaus provides two nodes for image handling: `ImageNode` for permanent images with remote URLs, and `LoadingImageNode` for optimistic previews during upload. Together they implement the upload flow described in the [Images & Uploads guide](/docs/guides/images-and-uploads).
