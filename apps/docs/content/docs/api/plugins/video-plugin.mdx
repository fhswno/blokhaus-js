...
```tsx
import { VideoPlugin } from "@blokhaus/core";
```
...
<Callout type="warn">
  Video embeds use iframes. The plugin sets `sandbox="allow-scripts
  allow-same-origin"` and `loading="lazy"` on all embed iframes for security and
