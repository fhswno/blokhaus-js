...
<Callout type="info">
  `AutoLinkPlugin` uses `AutoLinkNode` internally, which is distinct from the
  `LinkNode` used by `LinkPlugin`. Both node types are registered automatically
  when you use the respective plugins. They can coexist without conflict.
</Callout>
