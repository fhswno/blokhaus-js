...
<Callout type="warn">
  Never use base64 encoding for images. Base64 bloats the JSON state, hits
  database column size limits, and makes serialized state unreadable. Always
  upload to a remote URL via the `UploadHandler`.
</Callout>

<Callout type="info">
  Non-image files dropped onto the editor are silently ignored by `ImagePlugin`.
  If the dropped content is HTML or plain text, it falls through to the paste
  pipeline.
</Callout>
