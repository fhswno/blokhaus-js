...
<Callout type="info">
  This means you only include what you need. No prop explosion, no `<Editor enableAI enableImages enableTables />`. Remove a line, remove a feature.
</Callout>
...
<Callout type="warn">
  The `useEditorState` hook must be rendered **inside** `EditorRoot` since it needs access to the Lexical editor context.
</Callout>
